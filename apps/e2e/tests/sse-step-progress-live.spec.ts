/**
 * SSE pushes drive the running-AgentCard state without page reloads.
 *
 * Boundary: open ticket detail with no prior reviews, dispatch a webhook,
 * watch the AgentCards transition states without a manual refresh. We assert
...
test("AgentCard state transitions live without reload", async ({ page }) => {
  ...
  // Without refreshing the page, all three agents reach the terminal
  // `posted` state — the transition is driven by SSE invalidations of
