/**
 * The memory loop entry-point: a posted finding → Teach yaaof → new lesson.
 *
 * Boundary: clicking "Teach yaaof…" on a finding opens the modal pre-filled
...
test("Teach yaaof from a finding creates a lesson on the finding's repo", async ({ page }) => {
  ...
  await page.getByTestId("teach-yaaof").first().click();
