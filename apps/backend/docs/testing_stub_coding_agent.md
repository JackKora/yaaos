...
- **`review`** — ignores workspace. Emits one synthetic `Finding` (`file=src/example.ts`, `line_start=1`, `severity="suggestion"`, title/body tagged with `[stub] <agent_name>`) so UI flows that depend on a non-empty findings list have something to act against. Returns `state="COMMENT"` (not `CHANGES_REQUESTED`) — decoration, not a real must-fix.
- **`reply`** — ignores workspace. Returns `[stub] <agent_name> reply`.
...
`_STUB_TELEMETRY` is a module-level constant: `tokens_in=1000`, `tokens_out=200`, `cost_usd=Decimal("0.0050")`, `latency_ms=10`. Reused across all calls; visible in the audit log so specs can recognize stub-generated reviews.
...
- `test_wrapper.py` — stub `review` / `reply` return canned shapes; `validate_config` delegates; `health_check` returns stub mode; `wrap_all_registered_plugins` is idempotent and replaces every registry entry with a stub mirroring its `meta`.
