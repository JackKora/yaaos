...
Exports `AuditEntry`, `AuditEntryRow`, `AuditEntryNotFoundError`, generic `audit`, per-entity helpers (`audit_for_ticket`, `audit_for_pr`, `audit_for_reviewer_agent`, `audit_for_lesson`, `audit_for_review_job`, `audit_for_webhook_event`, `audit_for_workspace`), and reads (`list_for_entity`, `get`). See `apps/backend/app/core/audit_log/__init__.py`.
