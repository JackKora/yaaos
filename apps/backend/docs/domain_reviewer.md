...
3. `CodeReview` resolves the workspace, fetches the ticket context, builds an MCP payload via `mcp_wiring.build_mcp_payload`, invokes `coding_agent.review(plugin_id="claude_code", ...)`, and emits `draft_findings` + `summary_body` + `state` for `PostFindings`. The `on_activity` callback publishes `ActivityEvent`s to `core/sse` channel-per-workflow-execution.
