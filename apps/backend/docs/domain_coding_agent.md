...
- Types — `ReviewContext`, `ReviewResult`, `InvocationStatus`, `InvocationTelemetry`, `ValidationResult`, `HealthStatus`.
- Protocol — `CodingAgentPlugin`.
- Registry/dispatch — `register_coding_agent_plugin`, `get_plugin`, `registered_plugin_ids`, `review`, `validate_config`, `health_check_all`, `_reset_plugins_for_tests`, `_PLUGINS`.
...
Async methods `review`, `validate_config`, `health_check` plus `meta: PluginMeta`. Signatures in `app/domain/coding_agent/types.py`.

`review` MUST NOT raise on agent-level failures (timeout, non-zero exit, malformed JSON) — those become `status` + `error_message` so consumers branch on the same surface. Only infrastructure failures (e.g., `WorkspaceExecError`) are raised.
