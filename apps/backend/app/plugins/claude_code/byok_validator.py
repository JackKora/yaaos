"""Anthropic API key validator for `core/byok`.

The validator runs at request time when the BYOK UI's "Test key" button is
...
log = structlog.get_logger("plugins.claude_code.byok_validator")
...
async def validate_anthropic_key(plaintext: str) -> bool:
    ...
    matching the BYOK UX of "Test failed; check the key and try again".
    """
    headers = {
        "x-api-key": plaintext,
