...
- **Phase 6 follow-on slice 65 (this)** — `RealHandler` wired into `agent workspace`. Real tempdir lifecycle, real WriteFiles (with `../` escape protection), real auth-token refresh, real `os.RemoveAll` on cleanup. Configurable via `YAAOS_WORKSPACE_ROOT` env. Git clone inside CreateWorkspace + Claude Code subprocess for InvokeClaudeCode + wall-clock timeout + disk janitor + production OTel exporter still pending.
