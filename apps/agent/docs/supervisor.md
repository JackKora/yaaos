...
- **Short-poll guard for in-flight dispatch goroutines** — `buildClaimRequest()` uses `wait_seconds=1` when `inFlightDispatch > 0`. This counter is incremented before `go s.dispatch(...)` and decremented inside `dispatch` after `routeCommand` returns. Without this guard, the claim loop would re-arm with an empty `workspace_ids` (the workspace isn't in the pool yet) and issue a 30-second long-poll that misses the InvokeClaudeCode command queued immediately after ProvisionWorkspace completes — causing a 30-second stall per step in sequential multi-step workflows.
