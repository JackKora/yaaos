...
| **Activity event** | High-frequency CodingAgent telemetry event flowing through the `WSS /api/v1/agents/{id}/activity` WebSocket → `core/sse` → per-workflow SSE → UI. Never persisted; demand-pull (no events flow unless a UI tab is subscribed). |
