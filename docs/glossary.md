...
| **AgentCommand** | Wire message backend → WorkspaceAgent. Six kinds: `ProvisionWorkspace`, `WriteFiles`, `RefreshWorkspaceAuth`, `InvokeClaudeCode`, `CleanupWorkspace`, `EnumerateSkills`. Defined in `apps/backend/openapi/agent-api.yaml`. Persisted in `agent_commands` (durable queue). |
