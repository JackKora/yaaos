...
- **MCP context for coding agents (M04+)** — reviewer agents need business context (Linear tickets, Notion docs, Sentry errors) to grade intent, not just style. Per-user OAuth via a yaaos-hosted MCP proxy: yaaos brokers OAuth with each provider, holds tokens, exposes a per-review MCP URL the workspace points Claude Code at; workspace never sees upstream creds. Unblocks the workspace-egress-firewall trust boundary (Pattern B in the note). → [details](notes/mcp-context.md)
