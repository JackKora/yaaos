...
| 14 | [tickets-frontend.md](tickets-frontend.md) | Ticket list + detail UI; live-update flow (SSE → TanStack Query cache); how the ticket page composes data from `tickets` + `pull_requests` + `reviewer` backend modules. **Shipped 2026-05-16** — see `apps/web/src/domain/tickets/index.tsx` (single file, list + detail + Teach-yaaof modal) and `apps/web/src/core/sse/` (single EventSource at app root translating events into Query-cache invalidations). |
| 15 | [testing.md](testing.md) | Self-contained Docker test stack: `apps/fake-github` contract, `app/testing/e2e_setup` (programmatic reset + seed HTTP surface), `docker-compose.test.yml`, CLI cache (`CachingCodingAgentPlugin` + pytest fixture), test secrets, time-control env vars. Read after all backend modules — it ties their test surfaces together. |
